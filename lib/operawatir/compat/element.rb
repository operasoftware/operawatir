class OperaWatir::Element
  ...
  # @return [Boolean] true if the attribute exists on the element, false otherwise.
  def attr?(name)
  ...
  # On elements of type `<input>` this gets the value of the value attribute, on
  # every other element type it returns the text content
  # @return [String]
  def value
    if tag_name == 'INPUT'
      attr(:value)
  ...
  # Checks whether the text content of the element contains the given string
  # In the compatibility layer as the preferred way of doing this is
  #   elm.text.should include('My string')
  # @param [String] String to search for
  # @param [Boolean] true if the element's text contains str, false otherwise
  ...
  # @param [optional, Fixnum] x The offset from the left of the element
  ...
  # If passed a value it will type text into the element, otherwise it will
  ...
  # @return [OperaWatir::Collection] A collection of the selected `<option>`s
  def selected_options
    options(:selected?, true)
  end

  # On checkboxes, radio buttons, and option elements returns whether the
  # element is checked/selected. On a select element, when passed an value it
  # checks whether the selected option contains the given text
  # @param [optional, String] value Text the selected option should contain
  # @return [Boolean] true if the element is selected/selected option contains
  ...
  # For `<select>` elements returns either 'select-one' for `<select>`s where
  # only a single `<option>` can be selected, or 'select-multiple' otherwise. For
  # non-`<select>` elements returns the `type` attribute
  def type
    if tag_name == 'SELECT'
