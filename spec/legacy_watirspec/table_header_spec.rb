  ...
  describe "page" do
    describe "#exists?" do
      it "returns true if the table theader exists (page context)" do
        browser.thead(:id, 'tax_headers').exists?.should be_true
      end
    ...
    describe "how" do
      ...
      it "can be :index" do
        browser.thead(:index, 1).exists?.should be_true
      end

      it "can be :xpath" do
        browser.thead(:xpath, "//thead[@id='tax_headers']").exists?.should be_true
      end

      it "returns the first thead if given no args" do
        browser.thead.exists?.should be_true
      end
      it "defaults to :id" do
        browser.thead("tax_headers").exists?.should be_true
      end
  ...
  describe "table" do
    describe "#exists?" do
      it "returns true if the table theader exists (table context)" do
        browser.table(:index, 1).thead(:id, 'tax_headers').exists?.should be_true
      end
    ...
    describe "how" do
      ...
      it "can be :index" do
        browser.table(:index, 1).thead(:index, 1).exists?.should be_true
      end

      it "can be :xpath" do
        browser.table(:index, 1).thead(:xpath, "//thead[@id='tax_headers']").exists?.should be_true
      end

      it "defaults to :id" do
        browser.table(:index, 1).thead("tax_headers").exists?.should be_true
      end
